...
class Main(tk.Frame):
    ...
    def update_record(self, name, tel, email, plata):
        self.db.c.execute('''Update db SET name=?, tel=?, email=?, plata=? WHERE ID=?''',
                          (name, tel, email, plata, self.tree.set(self.tree.selection()[0],'#1')))
        self.db.conn.commit()
        self.view_records()
    ...
    def init_main(self):
        ...
        # добавлем параметры колонкам
        self.tree.column('ID', width=30, anchor=tk.CENTER)
        self.tree.column('name', width=210, anchor=tk.CENTER)
        self.tree.column('tel', width=120, anchor=tk.CENTER)
        self.tree.column('email', width=150, anchor=tk.CENTER)
        self.tree.column('plata', width=120, anchor=tk.CENTER)
    ...
    def records(self, name, tel, email, plata):
        self.db.insert_data(name, tel, email, plata)
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_num = tk.Label(self, text='Заработная плата')
        label_num.place(x=50, y=140)

        # добавляем строку ввода для наименования
        self.entry_name = ttk.Entry(self)
        # меняем координату обьекта
...
class Update(Child):
    ...
    def default_data(self):
        ...
        # получаем доступ к первой записи выборки
...
class DB:
    ...
    def insert_data(self, name, tel, email, plata):
        self.c.execute('''INSERT INTO db(name, tel, email, plata)
        VALUES (?, ?, ?, ?)''', (name, tel, email, plata))
        self.conn.commit()
